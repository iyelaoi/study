 ...
 * 算法正确，效率挺高
 */
public class Main81 implements Runnable{
    ...
    void search(){
        ...
        for (Integer twoId : set.get(nowRoot)){
            ...
            for (Integer threeId: set.get(twoId)){
                if (threeId <= nowRoot.intValue()) continue;
                ...
                for (Integer fourId: fourIds){
                    ...
                    if (fourId == nowRoot.intValue()){ // 如果第四层是根节点，直接加入结果集
                        result[0].add(tempList.toArray(CASE_FORMAT));
                        count++;
                        continue;
                    }
                    if (fourId.intValue() == twoId || fourId.intValue() == threeId){
                        continue;
                    }
                    /**
                    ...
                    if(layer42 != null){
                        ...
                        if(linkedList != null){
                            for (Integer midId: linkedList){
                                if (midId.intValue() == twoId || midId.intValue() == threeId){
                                    continue;
                                }
                                tempList.add(midId);
                    ...
                    for (Integer fiveId: set.get(fourId)){
                        ...
                        if (fiveId.intValue() == twoId || fiveId.intValue() == threeId){
                            /**
                             * 第五层节点与之前路径冲突
                             */
                            continue;
                        ...
                        if(layer52 != null){
                            ...
                            if (linkedList != null){
                                for (Integer mid: linkedList){
                                    if (mid.intValue() == twoId || mid.intValue() == threeId || mid.intValue() == fourId){
                                        continue;
                                    }
                                    tempList.add(mid);
    ...
    static void solve(){
        ...
        System.out.println("layer time: " + (System.nanoTime()-s)/1000/1000 + " MS");
    }

    private static void sharding(){
        int nodeSize = nodeSet.size();
        int s1 = nodeSize/21;
        int s2 = nodeSize/14 + s1;
        int s3 = nodeSize/7 + s2 - s1;
        int i = 0;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        //String name = "54";
        //String name = "3738";
        //String name = "77409"; // 重点学习
        //String name = "2896262";
        //String name = "2861665";
        //String name = "2755223";
        //String name = "1004812";
        //loadFile("D:\\wqz\\huawei\\first_game\\2020HuaweiCodecraft-TestData-master\\"+ name + "\\test_data.txt", false);
        loadFile("/data/test_data1.txt", false);
        long r = System.nanoTime();
        clear1();
        clear2();
        solve();
        sharding();
        Main81[] main81s = new Main81[THREAD_COUNT];
        for (int i = 0; i < THREAD_COUNT; i++){
            main81s[i] = new Main81(i);
            new Thread(main81s[i]).start();
        }
        try {
        ...
        out("/projects/student/result.txt", main81s);
    ...
    static void out1(String name, Main81[] mains){
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("" + totalCount.get() + LINE_SEPARATOR);
        for (int i = 0; i < 5; i++) {
            for (Main81 main : mains){
    ...
    static void out(String name, Main81[] main81s){
        long s = System.nanoTime();
        ...
        try{
            ...
            StringBuilder stringBuilder = new StringBuilder();
            for (int i = 0; i < 5; i++) {
                for (Main81 main81 : main81s){
                    List<Integer[]> linkedLists = main81.result[i];
                    if (linkedLists != null && linkedLists.size()>0) {
                        for (Integer[] ids : linkedLists) {
                            stringBuilder.append(ids[0]);
                            for (int n = 1; n < ids.length; n++){
