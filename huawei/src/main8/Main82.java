 ...
 * 转为数组
 */
public class Main82 implements Runnable{
    ...
    public static void main(String[] args){
        ...
        solve();
        sharding();
        Main82[] main82s = new Main82[THREAD_COUNT];
        for (int i = 0; i < THREAD_COUNT; i++){
            main82s[i] = new Main82(i);
            new Thread(main82s[i]).start();
        }
        try {
        ...
        threeLayer.clear();
        nodeSet.clear();
        out("/projects/student/result.txt", main82s);
    ...
    static void out1(String name, Main82[] mains){
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("" + totalCount.get() + LINE_SEPARATOR);
        for (int i = 0; i < 5; i++) {
            for (Main82 main : mains){
    ...
    static void out(String name, Main82[] main82s){
        long s = System.nanoTime();
        ...
        try{
            ...
            StringBuilder stringBuilder = new StringBuilder();
            for (int i = 0; i < 5; i++) {
                for (Main82 main82 : main82s){
                    List<Integer[]> linkedLists = main82.result[i];
                    if (linkedLists != null && linkedLists.size()>0) {
                        for (Integer[] ids : linkedLists) {
                            stringBuilder.append(ids[0]);
                            for (int n = 1; n < ids.length; n++){
